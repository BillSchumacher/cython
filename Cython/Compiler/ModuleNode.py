...
class ModuleNode(Nodes.Node, Nodes.BlockNode):
    ...
    def generate_h_code(self, env, options, result):
        ...
        if h_types or h_vars or h_funcs or h_extension_types:
            ...
            h_code_main.putln(
                f"/* WARNING: {warning_string.rstrip('.')} from Python 3.5 */"
            )
            ...
            h_code_main.putln(
                f"[[deprecated({warning_string.as_c_string_literal()})]] inline"
            )

            h_code_main.putln("#elif defined(__GNUC__) || defined(__clang__)")
            h_code_main.putln(
                f'__attribute__ ((__deprecated__({warning_string.as_c_string_literal()}), __unused__)) __inline__'
            )

            h_code_main.putln("#elif defined(_MSC_VER)")
            h_code_main.putln(
                f'__declspec(deprecated({warning_string.as_c_string_literal()})) __inline'
            )
            ...
            h_code_main.putln(
                f'#define {py3_mod_func_name}() __PYX_WARN_IF_{py3_mod_func_name}_INIT_CALLED({py3_mod_func_name}())'
            )
    ...
    def generate_public_declaration(self, entry, h_code, i_code):
        h_code.putln(
            f"{Naming.extern_c_macro} {entry.type.declaration_code(entry.cname)};"
        )
    ...
    def generate_api_code(self, env, options, result):
        ...
        if api_vars or api_funcs or api_extension_types:
            ...
            h_code.putln(
                f'module = PyImport_ImportModule({env.qualified_name.as_c_string_literal()});'
            )
    ...
    def generate_cclass_header_code(self, type, h_code):
        h_code.putln(
            f'{Naming.extern_c_macro} {PyrexTypes.public_decl("PyTypeObject", "DL_IMPORT")} {type.typeobj_cname};'
        )

    def generate_cclass_include_code(self, type, i_code):
        i_code.putln(f"cdef extern class {type.module_name}.{type.name}:")
        i_code.indent()
        if var_entries := type.scope.var_entries:
    ...
    def generate_c_code(self, env, options, result):
        ...
        code.putln(
            f'#define __Pyx_MODULE_NAME {self.full_module_name.as_c_string_literal()}'
        )
        ...
        code.putln("")
        code.putln(
            f"/* Implementation of {env.qualified_name.as_c_string_literal()} */"
        )
    ...
    def generate_declarations_for_modules(self, env, modules, globalstate):
        ...
        for module in modules:
            ...
            modulecode.putln("")
            modulecode.putln(
                f"/* Module declarations from {module.qualified_name.as_c_string_literal()} */"
            )
