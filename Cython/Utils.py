...
def path_exists(path):
    ...
    try:
        loader = __loader__
        if archive_path := getattr(loader, 'archive', None):
...
def find_versioned_file(directory, filename, suffix,
    ...
    matching_files = glob.glob(f"{path_prefix}.cython-*{suffix}")
    ...
    for path in matching_files:
        if versions := _parse_file_version(path):
...
def detect_opened_file_encoding(f, default='UTF-8'):
    ...
    if m and m.group(1) != b'c_string_encoding':
        ...
        if m := _match_file_encoding(lines[1]):
            return m.group(2).decode('iso8859-1')
    return default
...
def print_captured(captured, output, header_line=None):
    if captured := prepare_captured(captured):
        if header_line:
...
def write_depfile(target, source, dependencies):
    ...
    with open(f'{target}.dep', 'w') as outfile:
